...
export const resetPassword = async (req, res) => {
  ...
  sendMail(email, EmailSubject.reset, EmailType.reset, confirmedLink)
...
export const setPassword = async (req, res) => {
  ...
  sendMail(user.email, EmailSubject.set, EmailType.set, "", "", user.email, ip, date);

  res
    .status(200)
    .json({
      message: 'Your password has been changed successfully. Use your new password to login.'
    })
}
